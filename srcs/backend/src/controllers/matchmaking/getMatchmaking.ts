...
import { or, eq, and, ne, inArray, desc } from 'drizzle-orm';
...
export const getMatchmakingData = async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
        ...
        const categorizeUsers = (users: MatchmakingUser[], currentUserStats: { pongWins: number, pongLosses: number, snakeWins: number, snakeLosses: number }) => {
            ...
            users.forEach(user => {
                ...
                else if (Math.abs(user.winrate - (user.total_games > 0 ? user.win / user.total_games : 0)) <= 0.2) {
                    equalSkill.push(user);
                }
                // Check for similar game experience (within 5 games difference)
                else if (Math.abs(user.total_games - user.total_games) <= 5) {
        ...
        const currentUserStats = {
            pongWins: currentUserPongMatches.filter(match => 
                (match.p1_uuid === userUuid && match.status === matchStatus.P1_WINNER) ||
                (match.p2_uuid === userUuid && match.status === matchStatus.P2_WINNER)
            ).length,
            pongLosses: currentUserPongMatches.length - currentUserPongMatches.filter(match => 
                (match.p1_uuid === userUuid && match.status === matchStatus.P1_WINNER) ||
                (match.p2_uuid === userUuid && match.status === matchStatus.P2_WINNER)
            ).length,
            snakeWins: currentUserSnakeMatches.filter(match => 
                (match.p1_uuid === userUuid && match.winner_id === eWinner.PLAYER1) ||
                (match.p2_uuid === userUuid && match.winner_id === eWinner.PLAYER2)
            ).length,
            snakeLosses: currentUserSnakeMatches.length - currentUserSnakeMatches.filter(match => 
                (match.p1_uuid === userUuid && match.winner_id === eWinner.PLAYER1) ||
                (match.p2_uuid === userUuid && match.winner_id === eWinner.PLAYER2)
            ).length
        };
        ...
        const pongCategories = categorizeUsers(pongUsers, currentUserStats);
        const snakeCategories = categorizeUsers(snakeUsers, currentUserStats);
